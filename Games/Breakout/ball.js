class Ball {
	...
	checkForBrickCollisions(bricks) {
		let ballBrickCol = Math.floor(
			(this.xpos + this.size / 2) / bricks.brickWidth
		);
		let ballBrickRow = Math.floor(
			(this.ypos + this.size / 2) / bricks.brickHeight
		);
		let brickIndexUnderBall = bricks.rowColToArrayIndex(
			ballBrickCol,
			ballBrickRow
		);
